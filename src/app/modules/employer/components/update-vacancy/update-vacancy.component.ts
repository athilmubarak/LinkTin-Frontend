...
export class UpdateVacancyComponent implements OnInit {
    ...
    constructor(
        ...
        this.vacancy_form = this.form_builder.group({
            ...
            expected_notice_period: new FormControl(30, [
                Validators.required,
                Validators.min(1),
            ]),
            application_starts_from: new FormControl(
                new Date(),
                Validators.required
            ),
    ...
    saveVacancy() {
        ...
        const vacancy: VacancyRequest = {
            ...
            required_skills: this.data
                ? undefined
                : this.required_skills.map((x) => x.skill_id),
        };
    ...
    filterJobs(value: string) {
        if ([undefined, null, '', ' '].includes(value)) {
            ...
            this.jobs = this.shared_service.jobs.filter((x) =>
    ...
    showDoneButtonForJob(): boolean {
        return (
            ![null, undefined, '', ' '].includes(
                this.vacancy_form.get('job').value
            ) && this.jobs.length === 0
        );
    }
    ...
    saveNewJob() {
        this.shared_service
            .createNewJob({ name: this.vacancy_form.get('job').value })
            .subscribe({
    ...
    filterSkills(value: string) {
        if ([undefined, null, '', ' '].includes(value)) {
            ...
            this.skills = this.shared_service.skills.filter((x) =>
    ...
    showDoneButtonForSkill(): boolean {
        return (
            ![null, undefined, '', ' '].includes(
                this.vacancy_form.get('skill').value
    ...
    skillSelected(skill: Skill) {
        const index = this.required_skills.findIndex(
